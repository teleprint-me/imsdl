...
void* aligned_malloc(size_t alignment, size_t size) {
    ...
    if ((alignment & (alignment - 1)) != 0) {
        LOG_ERROR("Alignment %zu is not a power of 2.", alignment);
        return NULL;
    }
    ...
    if (ptr == NULL) {
        LOG_ERROR("_aligned_malloc failed (alignment=%zu, size=%zu).", alignment, size);
    }
#else
    int result = posix_memalign(&ptr, alignment, size);
    if (result != 0) {
        LOG_ERROR(
            "posix_memalign failed (alignment=%zu, size=%zu, error=%d).", alignment, size, result
        );
        return NULL;
    }
#endif
